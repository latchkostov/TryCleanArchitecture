...
To update the database use this command from the Web project folder (replace `TryCleanArchitecture` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../TryCleanArchitecture.Infrastructure/TryCleanArchitecture.Infrastructure.csproj -s TryCleanArchitecture.Web.csproj
```
